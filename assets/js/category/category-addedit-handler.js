				...
				_save: function(){
					...
					var success = function(response){
						if(response.status == 'OK'){
							...
							alertMessage('success', 'Simpan kategori berhasil');
							location.href = options.baseUrl + 'category';
							...
							if(typeof response.message){
								if(response.message.length > 0){
									...
									bootbox.alert(ERROR, 'Simpan kategori gagal');
								}
							}else{
								bootbox.alert(ERROR, 'Simpan kategori gagal');
							}
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'category/save',
						postdata: postdata,
						success: success,
						error: error
					});
