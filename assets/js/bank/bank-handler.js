			...
			var handler = {
				...
				_initTable: function(){
					...
					dataTable = $('#table-list').DataTable({
						...
						"serverSide": true,
						"ajax":{
							url : options.baseUrl + "bank/getBankList",
							type: "POST",
						...
						"columnDefs": [
							...
							},
							{
								"render": function ( data, type, row ) {
     								return '<a href="' + options.baseUrl + 'bank/edit/' + row[0] + '">'+data+'</a>';
								},
								"targets": 1
							},
							{
				...
				_delete: function(id){
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'bank/delete',
						postdata: postdata,
						success: success,
						error: error
					});
				...
				_clickListener: function(){
					...
					$('#table-list').on('click', '.edit', function(){
						var rowdata = dataTable.row($(this).parents('tr')).data();
						location.href = options.baseUrl + 'bank/edit/'+rowdata[0];
					});
				},
