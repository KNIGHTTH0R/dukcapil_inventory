			...
			var handler = {
				...
				_initTablePenerimaan: function(){
					...
					dataTable = $('#penerimaan-list').DataTable({
						...
						"serverSide": true,
						"ajax":{
							url : options.baseUrl + "kasir/getPenerimaanList",
							type: "POST",
						...
						"columnDefs": [
							...
							},
							{
								"render": function ( data, type, row ) {
     								return '<a href="' + options.baseUrl + 'kasir/edit_penerimaan/' + row[0] + '">'+data+'</a>';
								},
								"targets": 1
							},
							{
				...
				_initTablePengeluaran: function(){
					...
					dataTable2 = $('#pengeluaran-list').DataTable({
						"processing": true,
						"serverSide": true,
						"ajax":{
							url : options.baseUrl + "kasir/getPengeluaranList",
							type: "POST",
							error: function(response){
								if(response.status == 401)
									location.href = BASE_URL + 'login/expired'
								else{
									$("#pengeluaran-list tbody").remove();
									$("#pengeluaran-list").append('<tbody class="employee-grid-error"><tr><th colspan="4">No data found in the server</th></tr></tbody>');
									$("#pengeluaran-list_processing").css("display","none");
								}
							}
						},
						"columnDefs": [
							...
							{
								"render": function ( data, type, row ) {
     								return '<a href="' + options.baseUrl + 'kasir/edit_pengeluaran/' + row[0] + '">'+data+'</a>';
								},
								"targets": 1
							},
							{
				...
				_deletePengeluaran: function(id){
					var self = this;
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'kasir/deletePengeluaran',
						postdata: postdata,
						success: success,
						error: error
					});
				...
				_delete: function(id){
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'kasir/deletePenerimaan',
						postdata: postdata,
						success: success,
						error: error
					});
				...
				_clickListener: function(){
					...
					$('#pengeluaran-list').on('click', '.delete', function(){
						var rowdata = dataTable2.row($(this).parents('tr')).data();
						bootbox.confirm('Anda yakin ingin menghapus data ini ?', function(result){
							if(result){
								self._deletePengeluaran(rowdata[0]);
							}
						});
					});
					
					$('#pengeluaran-list').on('click', '.edit', function(){
						var rowdata = dataTable2.row($(this).parents('tr')).data();
						location.href = options.baseUrl + 'kasir/edit_pengeluaran/'+rowdata[0];
					});
					...
					$('#penerimaan-list').on('click', '.edit', function(){
						var rowdata = dataTable.row($(this).parents('tr')).data();
						location.href = options.baseUrl + 'kasir/edit_penerimaan/'+rowdata[0];
					});
				},
