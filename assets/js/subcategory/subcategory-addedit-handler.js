		...
		createIt: function(options){
			...
			var handler = {
				...
				_checkCode: function(){
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'subcategory/checkCode',
						postdata: postdata,
						success: success,
						error: error
					});
				...
				_save: function(){
					...
					var success = function(response){
						if(response.status == 'OK'){
							...
							location.href = options.baseUrl + 'subcategory';
							...
							if(typeof response.message){
								if(response.message.length > 0){
									...
									bootbox.alert(ERROR, 'Simpan sub kategori gagal');
								}
							}else{
								bootbox.alert(ERROR, 'Simpan sub kategori gagal');
							}
					...
					var postdata = {
						subcategory_id: $('#subcategory-id').val(),
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'subcategory/save',
						postdata: postdata,
						success: success,
						error: error
					});
