		...
		createIt: function(options){
			...
			var handler = {
				...
				_checkCode: function(){
					...
					var postdata = {
						distribusi_id: $('#distribusi-id').val(),
						code: $('#code').val()
					}
					
					LumiRequest.sendApi({
						url: options.baseUrl + 'distribusi_zakat/checkCode',
						postdata: postdata,
						success: success,
						error: error
					});
				...
				_save: function(){
					...
					var success = function(response){
						if(response.status == 'OK'){
							...
							location.href = options.baseUrl + 'distribusi_zakat';
							...
							if(typeof response.message){
								if(response.message.length > 0){
									...
									bootbox.alert(ERROR, 'Simpan distribusi zakat failed');
								}
							}else{
								bootbox.alert(ERROR, 'Simpan distribusi zakat failed');
							}
					...
					LumiRequest.sendApi({
						url: options.baseUrl + 'distribusi_zakat/save',
						postdata: postdata,
						success: success,
						error: error
					});
