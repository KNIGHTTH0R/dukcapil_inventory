...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    public function __construct($config = 'rest')
    {
		...
		header("Access-Control-Allow-Headers: secret-key, Origin, X-Requested-With, Content-Type, Accept, Access-Control-Request-Method");
		...
		$this->load->helper('toarray_helper');
		
		$this->key = base64_decode($this->config->item('default_key'));
		$this->iv = base64_decode($this->config->item('default_iv'));
    ...
    {		
        ...
        if (config_item('rest_enable_keys') and $use_key and $this->_allow === false) {
			...
			//ADJ CUSTOM CONDITION
			if($object_called != $this->config->item('token_method') && $object_called != $this->config->item('register_user_method')  && $object_called != $this->config->item('authenticate_method'))
				$this->response(array(config_item('rest_status_field_name') => 'ERR', config_item('rest_message_field_name') => 'Invalid API Key '.$this->rest->key), 401);
        }
        ...
        if (config_item('rest_enable_keys') and $use_key and !empty($this->rest->key) and !$this->_check_access()) {
            ...
            $this->response(array(config_item('rest_status_field_name') => 'ERR', config_item('rest_message_field_name') => 'This API key does not have access to the requested controller.'), 401);
        }

        // Sure it exists, but can they do anything with it?
        if ( ! method_exists($this, $controller_method)) {
            $this->response(array(config_item('rest_status_field_name') => 'ERR', config_item('rest_message_field_name') => 'Unknown method.'), 404);
        }
        ...
        if (config_item('rest_enable_keys') and !empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') and !$this->_check_limit($controller_method)) {
                $response = array(config_item('rest_status_field_name') => 'ERR', config_item('rest_message_field_name') => 'This API key has reached the hourly limit for this method.');
            ...
            $response = array(config_item('rest_status_field_name') => 'ERR', config_item('rest_message_field_name') => 'This API key does not have enough permissions.');
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
		...
		$arr['data'] = $this->encrypt_decrypt->encrypt(json_encode($data), $this->key, $this->iv);
		...
		if(is_array($output)){
			...
			$arr['status'] = 'ERR';
			$arr['message'] = 'Invalid POST format';
			
			$dt['data'] = $this->encrypt_decrypt->encrypt($arr, $this->key, $this->iv);
        ...
        if($continue){
            ...
            $arr['data'] = $this->encrypt_decrypt->encrypt($output, $this->key, $this->iv);
			exit(json_encode($arr));
			//exit($output);
        }
    }
    ...
    protected function _detect_api_key()
    {
		...
		if(is_array($this->_args['data'])){
			$this->response(array(config_item('rest_status_field_name') => 'ERR', 'message' => 'Invalid POST format'), 406);
		}
		
		$this->load->library('encrypt_decrypt');
		$this->_args = $this->encrypt_decrypt->decrypt($this->_args['data'], $this->key, $this->iv);
		$this->_args = toArray($this->_args);
		
		$api_key_variable = strtolower(str_replace('-','_',$api_key_variable));
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
			...
			$secretExpired = 0;
			$opt = 'KEY_EXPIRED_TIME';
		
			$sql = $this->db->prepare('SELECT option_value
										FROM pspot_setting
										WHERE option_key=:option_key
										LIMIT 1');
					
			$sql->bindParam(':option_key', $opt);
			$sql->execute();
			
			$row = $sql->fetch(PDO::FETCH_ASSOC);
			$secretExpired = intval($row['option_value']);	
			$sts = 'A';
			
			$sql = $this->db->prepare('SELECT pcs.status, pcs.modified_date
									   FROM pspot_client_security pcs
									   JOIN pspot_client pc ON pc.client_id = pcs.client_id
									   WHERE secret_key=:secret_key');
									   //WHERE refresh_key=:refresh_key');

			$sql->bindParam(':secret_key', $key);
			//$sql->bindParam(':refresh_key', $this->_args['refresh_key']);
			$sql->execute();
			...
			if($rows){
				...
				if($rows['status'] == 'N'){
					...
					if($method != $this->config->item('verify_method') && $method != $this->config->item('resend_method')){
						
						$this->response(array(config_item('rest_status_field_name') => 'ERR',
													  'message' => 'Your account not verified yet. Please input the code gived to you in your email'), 401);
				...
				if($rows['status'] == 'S'){
					...
					$this->response(array(config_item('rest_status_field_name') => 'ERR',
												  'message' => 'Your account has been suspended'), 401);
												  
					return false;
				}
				
				$diff = checkExpired($rows['modified_date']);
				
				if(floatval($diff) > floatval($secretExpired)){
					if(empty($this->_args['refresh_key'])){
						$this->response(array(config_item('rest_status_field_name') => 'ERR', config_item('rest_message_field_name') => 'Key Expired'), 401);
						return false;
					}else{
						
						$sql = $this->db->prepare('SELECT count(*) AS total 
													FROM pspot_client_security
													WHERE refresh_key=:refresh_key');
								
						$sql->bindParam(
							':refresh_key', $this->_args['refresh_key']
						); 
						...
						if($r['total'] <= 0){
							...
							$this->secret_key = generateUserKey();
							
							$sql->bindParam(':secret_key', $this->secret_key);
							$sql->bindParam(':modified_date', $rectime);
							$sql->bindParam(':refresh_key', $this->_args['refresh_key']);
							
							$q = $sql->execute();
							
							//$this->response(array(config_item('rest_status_field_name') => 'OK',
												  //'secret_key' => $this->secret_key), 200);
						}
					}
				}else{
					$this->secret_key = $this->_args['secret_key'];
				}
				
			}else{
				
				//CHECK ATTENDANT
				$sql = $this->db->prepare('SELECT ppak.modified_date
										   FROM pspot_parking_attendant_key ppak
										   JOIN pspot_parking_attendant ppa ON ppa.parking_attendant_id = ppak.parking_attendant_id
										   WHERE secret_key=:secret_key
										   AND status=:status');
    ...
    protected function _parse_post()
    {
		...
		if (count($this->_post_args) > 0) {
			parse_str(file_get_contents('php://input'), $post);
			$this->request->body = $post;
			
			$method = $_SERVER['REQUEST_URI'];
			$method = explode('/', $method);
			$method = end($method);	
			
			if(($this->_post_args == NULL || empty($this->_post_args)) && ($this->request->body == NULL || empty($this->request->body))){
				if($method != $this->config->item('token_method')){
					$this->response(array(config_item('rest_status_field_name') => 'ERR', 'message' => 'Invalid POST format. POST data must be array in PHP and object in javascript with object/array key "data"'), 406);
				}
			}
			
			if(!$this->request->body)
				$this->response(array(config_item('rest_status_field_name') => 'ERR', 'message' => 'Invalid JSON format"'), 406);
			
			if (!empty($this->request->body)) {
				$this->_post_args = array_merge($_POST, $this->request->body);
			}
			...
			if(!empty($this->request->body)){
				...
				if (!empty($this->request->body)) {
					$this->_post_args = array_merge($_POST, $arr);
				}
			}
		}
    ...
    {		
        if ($key === null) {
			...
			if(isset($this->_post_args['data'])){			
				$this->load->library('encrypt_decrypt');
				
				$dt = $this->_post_args['data'];
				
				if(is_array($dt)){
					$this->response(array(config_item('rest_status_field_name') => 'ERR', 'message' => 'Invalid POST format'), 406);
				}
				
				$this->_post_args = toArray($this->encrypt_decrypt->decrypt($dt, $this->key, $this->iv));
			}
